...
class Window(Frame):
    ...
    def __init__(self, master = None, puertos = None, baudios = 9600,
                 ...
                 dim_red_ir = 100, 
                 dimension2 = 50*2, dim_array2 = 1,dpi2 = 100,     ##config para pulso
 ...
 ################################################################ 
    ...
    ##
                ...
                if (self.i >= self.eje_x-2):
                    ...
                    self.y[self.i + self.dim_array] = np.nanmean(self.y)#None
                    self.y[self.i + self.dim_array + 1] = np.nanmean(self.y)#None
    ...
    def conexion(self):
        try:
            ...
            if (self.define == 1):
                ...
                self.firebase = firebase.FirebaseApplication('https://monitorsignosvit-default-rtdb.firebaseio.com/',None)
                self.fireConexion()
        ...
        while self.define:
            ...
            try:
                ...
                if (self.i2 >= self.eje_x2-2):#-2
                    ...
                    pass
                    #los dos ultimos valores seran vacios
                    self.y2[self.i2 + self.dim_array2] = np.nanmean(self.y2)# None
                    self.y2[self.i2 + self.dim_array2 + 1] = np.nanmean(self.y2)#None
                #print(self.y2,'l')
        ...
        while self.define:
            ...
            try:
                ...
                if (self.i4 >= self.eje_x4-2):#-2
                    ...
                    pass
                    #los dos ultimos valores seran vacios
                    self.y4[self.i4 + self.dim_array4] = np.nanmean(self.y4)# None
                    self.y4[self.i4 + self.dim_array4 + 1] = np.nanmean(self.y4)# None
                #print(self.y2,'l')
    ...
    def esperar5(self):
        while self.define:
            try:
                ...
                for key in datos:
                    #print(key,datos[key])
                    self.firebase.put('/prueba/datos_post/-NB8xDJejQG3fyEjT8wg',key,datos[key])
